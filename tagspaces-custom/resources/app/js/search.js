      ...
      TSCORE.IOUtils.walkDirectory(TSCORE.currentPath, {recursive: queryObj.recursive},
        function(fileEntry) {
          return new Promise(function(resolve, reject) {
            //search in tagify index
            var tags=TSCORE.TagUtils.tagify.getTags(fileEntry.path);
            console.error(tags,queryObj);
                
            if ((queryObj.includedTags.length?queryObj.includedTags:queryObj.includedTerms).filter(function(t){return tags.indexOf(t[0])!=-1}).length)
              if ((queryObj.excludedTags.length?queryObj.excludedTags:queryObj.excludedTerms).filter(function(t){return tags.indexOf(t[0])!=-1}).length==0)
        ...
        function(entries) {
          ...
          console.error("Found " + searchResults.length + " out of " + entries.length + " entries.");

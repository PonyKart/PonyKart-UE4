...
var Tagify = require('./js/tagify.js'),
    tagify = new Tagify('../index.json','../PonyKart Development Assets');
    
setTimeout(function(){tagify.reload();tagify.save();},1000);
...
define(function(require, exports, module) {
  ...
  function extractTitle(filePath) {
    ...
    console.error('extractTitle',filePath);
    return fileName;  //do not parse tags
  ...
  function extractTags(filePath) {
    ...
    var tags = tagify.getTags(filePath);
    //console.error('extractTAGS',filePath,tags);
    return tags;
    ...
    console.error("extractTags()",filePath,cleanedTags);
    return cleanedTags;
  }

  function suggestTags(filePath) {
    console.log('suggestTags' + filePath);
    
    ////////////////
    
    console.error('suggestTags',filePath);
    return [];  //do not suggest tags
  ...
  function generateFileName(fileName, tags) {
    ...
    throw new Error("illegal invocation",fileName,tags);
  ...
  function writeTagsToFile(filePath, tags) {
    ...
    console.error('writeTAGS',filePath,tags);
    //TODO:THIS
    tagify.addTags(filePath, tags);
    tagify.scheduleSave();
    return TSPOSTIO.renameFile(filePath, filePath);
  ...
  function removeTagsFromFile(filePath, tags) {
    ...
    console.error('removeTAGS',filePath,tags);
    //TODO:THIS
    tagify.removeTags(filePath,tags);
    tagify.scheduleSave();
    return TSPOSTIO.renameFile(filePath, filePath);
  ...
  function cleanFileFromTags(filePath) {
    ...
    console.error('cleanFile',filePath);
    tagify.removeTags(filePath,tagify.getTags(filePath)); //irrelevant
    tagify.scheduleSave();
    return TSPOSTIO.renameFile(filePath, filePath);
  ...
  function moveTagLocation(filePath, tagName, direction) {
    ...
    console.error('moveTAG',filePath,tagName,direction);
    return; //irrelevant
  ...
  function renameTag(filePath, oldTag, newTag) {
    ...
    //TODO: THIS
    console.error('renameTAG',filePath,oldTag,newTag);
    tagify.removeTags(filePath,[oldTag]);
    tagify.addTags(filePath,[newTag]);
    tagify.scheduleSave();
    return TSPOSTIO.renameFile(filePath, filePath);;
  ...
  function changeTitle(filePath, newTitle) {
    ...
    console.error('changeTITLE',filePath,newTitle);
  ...
  function removeTag(filePath, tagName) {
    ...
    console.error('removeTAG',filePath,tagName);
    
    //TODO:THIS
    tagify.removeTags(filePath,[tagName]);
    tagify.scheduleSave();
    return TSPOSTIO.renameFile(filePath, filePath);
